...
import {
  enviarMensajeTxt,
  enviarMensajeWsp,
} from "../transporters/mensajesTwilio.js";
import { sendMail } from "../transporters/nodeMailer.js";
import {
  addProductTo,
  carritoActivoByUserId,
  comprar,
  deleteProductFrom,
...
export const putCartAsClosed = async (req, res) => {
  try {
    ...
    await enviarMensajeWsp(
      `nuevo pedido de ${req.user.username}, email ${req.user.email}`
    ); // mensaje de whatsapp al administrador
    await enviarMensajeTxt(
      "Tu pedido se ha recibido y se encuentra en proceso",
      req.user.phone
