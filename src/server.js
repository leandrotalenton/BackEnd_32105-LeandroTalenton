...
if (method == "CLUSTER") {
    if (cluster.isPrimary) {
        ...
        HttpServer.listen(port, () => {
            logger.info(`servidor iniciado en el puerto ${port} con el metodo ${method}, en el worker ${process.pid}`)
        });
    }
} else {
    HttpServer.listen(port, () => {
        logger.info(`servidor iniciado en el puerto ${port} con el metodo ${method}`)
    })
}